...
@Injectable()
export class PersonsSkillsService {
    private resourceUrl = SERVER_API_URL + 'api/persons';

    constructor(private http: HttpClient) {}

    findAchievableSkill(personId: number, skillId: number, req?: any): Observable<IAchievableSkill> {
        const options = createRequestOption(req);
        return this.http
            .get<IAchievableSkill>(`${this.resourceUrl}/${personId}/achievable-skills/${skillId}`, {
                params: options,
                observe: 'response'
            })
    ...
    queryAchievableSkills(personId: number, req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http
            .get<IAchievableSkill[]>(`${this.resourceUrl}/${personId}/achievable-skills`, {
                params: options,
                observe: 'response'
            })
    ...
    updateAchievableSkill(personId: number, skill: IAchievableSkill): Observable<EntityResponseType> {
        const copy = this.convert(skill);
        return this.http
            .put<IAchievableSkill>(`${this.resourceUrl}/${personId}/achievable-skills`, copy, { observe: 'response' })
